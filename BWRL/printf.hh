...
template<typename... T>
std::basic_string<char> Printf(PrintfFormatter& fmt, const T&... args)
{
...
std::basic_string<char> Printf(const std::basic_string<CT>& format, const T&... args)
{
...
/* This is the function you would use. */
template<typename CT, typename... T>
std::basic_string<char> Printf(const CT* format, const T&... args)
{
	PrintfFormatter Formatter;
	Formatter.MakeFrom(format);
	return Printf(Formatter, args...);
...
struct PrintfProxy
{
	...
	PrintfProxy() : data(new std::remove_reference<decltype(*data)>::type) { }
